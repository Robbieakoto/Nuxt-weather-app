<template>
    <div class="bg-gray-700 items-center flex flex-col md:flex-row md:justify-center p-6 isolate px-6 py-24 sm:py-32 lg:px-8">
        <div>
            <form class="space-y-6" action="#" method="POST">
                <div>
                    <label for="location" class="block text-sm font-medium leading-6 text-white">Search Location</label>
                    <div class="mt-2">
                    <input id="location" name="location" type="location" autocomplete="location" placeholder="search" required class="block w-full rounded-md border-0 py-3 pl-7 pr-20 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
                    </div>
                </div>
            </form>
            <div class="items-center flex flex-col md:flex-row md:justify-center mt-20">
                <div class="w-64 md:mr-20 mb-10 transition duration-500 ease-in-out transform bg-white rounded-lg hover:scale-105 cursor-pointer border flex flex-col justify-center items-center text-center p-6">
                    <div class="text-md font-bold flex flex-col text-gray-900"><span class="uppercase">Today</span> <span class="font-normal text-gray-700 text-sm">{{weatherDetails.current.time}}</span></div>
                ...
                <p class="text-gray-700 mb-2">{{weatherDetails.current.cloudcover + weatherDetails.current_units.cloudcover}}</p>
                <div class="text-3xl font-bold text-gray-900 mb-6">{{ weatherDetails.daily.temperature_2m_max[0]+ weatherDetails.daily_units.temperature_2m_max}}<span class="font-normal text-gray-700 mx-1">/</span>{{ weatherDetails.daily.temperature_2m_min[0]+ weatherDetails.daily_units.temperature_2m_min}}</div>
            </div>

            <div class="w-64 md:mr-20 mb-10 transition duration-500 ease-in-out transform hover:scale-105 cursor-pointer border rounded-lg flex flex-col justify-center items-center text-center p-6 bg-gray-900">
                <div class="text-md font-bold flex flex-col text-white"><span class="uppercase">Today</span> <span class="font-normal text-white text-sm">{{weatherDetails.current.time}}</span></div>
                    <div class="w-32 h-32 flex items-center justify-center">
                        <svg width="95" height="72" viewBox="0 0 95 72" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <p class="text-white mb-2">{{weatherDetails.current.cloudcover + weatherDetails.current_units.cloudcover}}</p>
                    <div class="text-3xl font-bold text-white mb-6">{{ weatherDetails.daily.temperature_2m_max[0]+ weatherDetails.daily_units.temperature_2m_max}}<span class="font-normal text-white mx-1">/</span>{{ weatherDetails.daily.temperature_2m_min[0]+ weatherDetails.daily_units.temperature_2m_min}}</div>
                </div>
            </div>
        </div>
    </div>
...
<script>
    export default {
        mounted() {
            this.weatherDetails = this.getWeather();
        },
        data() {
            return {
                 weatherDetails: []
        ...
        methods:{
            getWeather() {
                fetch("https://api.open-meteo.com/v1/forecast?latitude=52.52&longitude=13.41&current=temperature_2m,is_day,precipitation,cloudcover&daily=weathercode,temperature_2m_max,temperature_2m_min,sunrise,rain_sum,showers_sum&timezone=GMT")
                    .then((res) => res.json())
                    .then((weatherDetails) => {
